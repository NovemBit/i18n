...
namespace i18n\component\translation\component\url;
...
class Component extends \NovemBit\i18n\system\Component {
	 ...
	 * @throws \Exception
	 */
	public function translate( array $urls ) {
		...
		$translations = $this->getPathPartTranslations( array_keys( $paths_to_translate ) );
	 ...
	 * @return array
	 */
	private function buildTranslateResult( array $urls, array $languages, array $translations ) {
		$result = [];
		foreach ( $urls as $url ) {
			 ...
			 * */
			$path_parts = isset( $url_parts['path'] ) ? $this->getPathParts( $url_parts['path'] ) : [];
