...
//        $this->url_validation_rules['path'][] = '^(?!^\/$).+$';
    ...
    {
        $url = ltrim($url, '/ ');
        $url = preg_replace("/^" . preg_quote($this->context->context->languages->getScriptUrl(), '/') . "/", '', $url);
    ...
    public function validateAfterReTranslate($before, $after, &$result)
    {
        ...
        $result[$before] = \NovemBit\i18n\system\helpers\URL::removeQueryVars($result[$before] ,$this->context->context->languages->language_query_key);
